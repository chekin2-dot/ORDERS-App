...
export default function ConversationScreen() {
    ...
    try {
      ...
      if (data) {
        ...
        setConversation({
          ...
          driver_name: `${driverData?.user_profiles?.first_name} ${driverData?.user_profiles?.last_name}`,
          driver_phone: driverData?.user_profiles?.phone || '',
          driver_whatsapp: driverData?.user_profiles?.whatsapp_number,
          driver_photo_url: driverData?.user_profiles?.profile_photo_url,
        });
      }
    } catch (error) {
  ...
  const subscribeToMessages = () => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (newMessage.sender_id !== profile?.id) {
            console.log('Playing notification for incoming message');
            markMessagesAsRead();
            ...
            setAlertMessage({
              ...
              sender: conversation?.driver_name || 'Livreur',
            });
      ...
      .subscribe((status) => {
        console.log('Client conversation subscription status:', status);
        if (status === 'SUBSCRIBED') {
          console.log('Successfully subscribed to conversation messages');
        } else if (status === 'CHANNEL_ERROR') {
          console.error('Channel error for conversation messages');
        }
      });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (conversation?.driver_whatsapp && profile.user_type === 'client') {
        const whatsappUrl = `https://wa.me/${conversation.driver_whatsapp}?text=${encodeURIComponent(messageText)}`;
  ...
  const shareLocation = async () => {
    if (!profile?.latitude || !profile?.longitude) {
      Alert.alert('Position non disponible', 'Veuillez d\'abord enregistrer votre position GPS dans les paramètres');
      return;
    }
    ...
    try {
      ...
      const locationMessageText = `📍 Ma position: ${mapUrl}`;
      ...
      const { error: locationError } = await supabase.from('order_messages').insert(locationMessageData);

      if (locationError) throw locationError;

      if (profile.whatsapp_number) {
        const whatsappMessageText = `📱 Mon WhatsApp: ${profile.whatsapp_number}`;
        ...
        const { error: whatsappError } = await supabase.from('order_messages').insert(whatsappMessageData);
        ...
        await supabase
          .from('order_conversations')
          .update({
            last_message: whatsappMessageText,
            last_message_at: new Date().toISOString(),
          })
          .eq('id', conversationId);
      } else {
        await supabase
          .from('order_conversations')
          .update({
            last_message: locationMessageText,
            last_message_at: new Date().toISOString(),
          })
          .eq('id', conversationId);
      }

      if (conversation?.driver_whatsapp) {
        const whatsappMessage = `Voici ma position: ${mapUrl}`;
        const whatsappUrl = `https://wa.me/${conversation.driver_whatsapp}?text=${encodeURIComponent(whatsappMessage)}`;
  ...
  const openLocation = (latitude: number, longitude: number) => {
    const url = Platform.select({
      ios: `maps:0,0?q=Position@${latitude},${longitude}`,
      android: `geo:0,0?q=${latitude},${longitude}(Position)`,
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        {conversation?.driver_photo_url ? (
          <Image
            source={{ uri: conversation.driver_photo_url }}
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.title}>{conversation?.driver_name || 'Chargement...'}</Text>
      ...
      >
        ...
        <TypingIndicator
          userName={conversation?.driver_name || 'Livreur'}
